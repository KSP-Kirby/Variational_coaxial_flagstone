...
params.b = 138.6757;        % from GC version 2016_08_25_coaxial-flagstone 
...
for j = 1:iterations
    for i = 1:length(rayOut1_f)
        [z0_est(i,:), rms_flow_error(i)] = gradientDescent(rayOut1_f(i,:)*params.pixelDim, rayOut1_b(i,:)*params.pixelDim, z0_est(i,:), params);
...
imgOut_f = mirrorHorz(radial2XY(z0_est,4))/6000;
depthMap = mirrorHorz(radial2XY(z0_est,4));

[m, n] = size(imgOut_f);
for i = 1:m
    for j = 1:n
        if isnan(imgOut_f(i,j)) 
            imgOut_f(i,j) = 0;
        end
